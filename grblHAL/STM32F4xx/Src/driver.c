...
#endif
#ifdef  SPINDLE_INDEX_EMULATE
#define SPINDLE_INDEX_BIT 0
#endif

#define DRIVER_IRQMASK (LIMIT_MASK|CONTROL_MASK|KEYPAD_STROBE_BIT|SPINDLE_INDEX_BIT)
//B12-B15 X_LIMIT_BIT|Y_LIMIT_BIT|Z_LIMIT_BIT
//B6-B9 CONTROL_RESET_BIT|CONTROL_FEED_HOLD_BIT|CONTROL_CYCLE_START_BIT|CONTROL_SAFETY_DOOR_BIT
//B15 KEYPAD_STROBE_BIT|PROBE_BIT|A_LIMIT_BIT
//B10 SPINDLE_INDEX_BIT
...
#ifdef SPINDLE_INDEX_EMULATE
    spindle_encoder.index_count--;
    if(spindle_encoder.index_count<=0){
    	spindle_encoder.index_count=spindle_encoder.ppr;
    	spindle_encoder.counter.last_index = cval;//RPM_COUNTER->CNT;
    	spindle_encoder.timer.last_index = tval;//RPM_TIMER->CNT;
    	spindle_encoder.counter.index_count++;
    }
#endif
